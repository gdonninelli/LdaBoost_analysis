...
def load_har(base: Path) -> Tuple[np.ndarray, np.ndarray]:
    ...
    x = ensure_numeric_features(df.drop(columns=["Activity"]))
    return x, y


def load_rainfall(base: Path) -> Tuple[np.ndarray, np.ndarray]:
    df = pd.read_csv(base / "rain" / "Data" / "train.csv")
    y = df["rainfall"].astype(int).to_numpy()
    x = ensure_numeric_features(df.drop(columns=["rainfall"]))
    return x, y
        ...
        for fold_id, (train_idx, test_idx) in enumerate(cv_outer.split(x_raw, y_enc), start=1):
            ...
            for method_name in selected_methods:
                ...
                if method_name == "GBM":
                    ...
                    best_model, best_params, best_inner_score = tune_with_grid_search(
                        estimator=base,
                        param_grid=gb_grid,
                        x_train=x_train,
                        y_train=y_train,
                    ...
                    best_model, best_params, best_inner_score = tune_ldaboost_manual(
                        x_train=x_train,
                        y_train=y_train,
                        inner_splits=inner_splits,
                        random_state=args.seed,
