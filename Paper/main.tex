...
Beyond computational gains, dimensionality reduction also mitigates the effects of the curse of dimensionality, which can degrade model performance \citep{hu2024tackling}. For example, classification trees are prone to overfitting when trained on data with many irrelevant features, often resulting in higher missclassification rates on test sets.
...
\DIFaddbegin
\DIFadd{Linear Programming Boosting (LPBoost; \citep{demiriz2002lpboost}) and FilterBoost \citep{bradley2008filterboost} are two variants of classical boosting that primarily modify how weak learners are selected and weighted. LPBoost reformulates boosting as a global optimization problem, closely related to margin-based methods such as support vector machines, with the goal of finding an optimal and often sparse combination of weak learners rather than adding them in a purely greedy, sequential manner as in Gradient Boosting Machines (GBM). FilterBoost, by contrast, is designed with scalability in mind: it introduces a filtering or sampling mechanism that allows the algorithm to operate on subsets of the training data at each iteration, thereby reducing computational cost while maintaining predictive performance. In both cases, however, the representation of the data remains unchanged. The algorithms operate directly on the original input features, without introducing new feature transformations beyond standard preprocessing steps. The key differences with respect to GBM therefore lie in the learning strategy rather than in the feature space.}

\DIFadd{Boosting with Kernel Projections \citep{kegl2009kernelboost} introduces a more substantial departure from the classical framework by incorporating a transformation of the input space prior to or within the boosting procedure. The central idea is to map the original features into a higher-dimensional or more expressive space through a kernel-based transformation, enabling the construction of weak learners that can capture more complex, nonlinear patterns. The boosting process then proceeds in this transformed space, iteratively combining weak learners to improve predictive accuracy on the target. Conceptually, this approach combines the representational power of kernel methods with the sequential error-correcting nature of boosting.}

\DIFadd{From the perspective of feature representation, this third approach differs fundamentally from both GBM and its variants such as LPBoost and FilterBoost. The transformed features are not simple linear combinations of the original inputs but arise from a kernel-induced mapping that may be implicit and high-dimensional. Importantly, this mapping is typically not constructed in a supervised manner with respect to the target; it is defined a priori by the choice of kernel function. However, the boosting procedure introduces a form of indirect supervision by progressively selecting and emphasizing those components of the transformed space that are most useful for predicting the target. In this sense, feature construction occurs in two stages: an initial, largely unsupervised expansion of the feature space, followed by a supervised selection driven by the boosting iterations. More advanced variants further blur this distinction by adapting or combining kernels during training, thereby allowing the representation itself to become partially target-dependent. This dual mechanism distinguishes kernel-based boosting approaches from classical GBM, where both representation and learning are typically confined to the original feature space.}
\DIFaddend
...
Thirdly, only for LdaBoost do we refit the algorithm, with parameters previously fixed to match the GBM estimates for ease of comparison.

For each dataset, algorithms were compared using 10-fold cross-validated accuracy, and for the HAR and Rainfall datasets, also by test accuracy (for HAR we use the test set available from UCI Machine Learning). 
...
\DIFaddbegin \IfFileExists{figures/accuracy_comparison.pdf}{
\begin{figure}[h!]
    \centering
...
From this perspective, the relabeling step can be viewed in connection with cost-sensitive and reweighted classification frameworks, where the learning problem is modified to emphasize difficult or misclassified observations \citep{elkan2001cost, zadrozny2003cost}, as well as with hard assignment strategies derived from soft signals, as in expectation--maximization (EM) algorithms \citep{dempster1977em}. To this end, LDA plays a dynamic role within this framework: at each iteration, it identifies a discriminant subspace that emphasizes the directions where the model is currently making the largest errors, as indicated by the gradient signal.
...
Best per row in \textbf{bold}.}
\label{tab:classification_results_old}
\begin{tabular}{@{}llcccc@{}}
\toprule
\multicolumn{2}{c}{\textbf{Dataset}} & \textbf{GBM} & \textbf{PCA+GBM} & \textbf{LDA+GBM} & \textbf{\textsc{LdaBoost}} \\
\midrule
...
\medskip
\raggedright\footnotesize
\textit{Note:} Bracketed values are test accuracy for \textsc{LdaBoost}.
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Cross-validated and test validation runtime (seconds). Best per row in bold.}
\label{tab:A1_runtime_summary}
\begin{tabular}{llcccc}
\toprule
& \textbf{Dataset} & \textbf{GBM} & \textbf{PCA+GBM} & \textbf{LDA+GBM} & \textbf{LdaBoost} \\
\midrule
\multirow{5}{*}{\textbf{CV}}
& IRIS     & 0.12     & 0.15     & \textbf{0.04} & 2.18     \\
& YEAST    & \textbf{2.38} & 3.65     & 3.12          & 16.42    \\
& HAR      & 429.84   & 407.42   & \textbf{9.18}  & 204.65   \\
& RAINFALL & \textbf{0.22} & 1.48     & 1.07          & 2.56     \\
& SONAR    & 0.14     & 0.19     & \textbf{0.05} & 2.24     \\
\midrule
\multirow{2}{*}{\textbf{Test time (s)}}
& HAR      & 675.07   & 652.62   & \textbf{10.25} & 286.22   \\
& RAINFALL & 0.53     & 0.70     & \textbf{0.27}  & 2.33     \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h!]
    ...
    \caption{\textit{Appendix replication for the binary target: accuracy as a function of the number of features ($\rho\approx0.5$, $N=10{,}000$).}}
    \label{fig:A7}
\end{figure}
\begin{figure}[h!]
    \centering
    ...
    \caption{Appendix comparison of the different pipelines in a low-correlation scenario.}
    \label{fig:fig7}
\end{figure}
\begin{figure}[h!]
    \centering
    ...
    \caption{Appendix comparison of the different pipelines in a high-correlation scenario.}
    \label{fig:fig8}
\end{figure}
