...
Given training features $(x_1, \dots, x_N)$, with $x_i \in \mathbb{R}^p$, PCA is defined without using class labels. For supervised methods introduced later, we denote labels by $y_i \in \{1,\dots,C\}$.  
...
G = [G_1 \ | \dots | \ G_r].
\]
Hence, the discriminant weights depend jointly on between-class mean separation and within-class variability (including covariance): a feature receives a larger coefficient only when it contributes to class separation relative to its within-class dispersion.
...
For $m>0$, let
\[
g^{(m)}_{i,c} = Y_{i,c} - P^{(m-1)}_{i,c}, \qquad
z^{(m)}_i = \arg\max_{c\in\{1,\dots,C\}} g^{(m)}_{i,c},
\]
where $Y$ is one-hot encoded and $P^{(m-1)}$ are current class probabilities. We then fit LDA on $(X, z^{(m)})$ and obtain $G^{(m)}$ by maximizing the Fisher ratio
\[
\max_G\ \frac{|G^\top S_B(z^{(m)})G|}{|G^\top S_W(z^{(m)})G|}.
\]
Therefore, Step 1 remains a valid Fisher optimization at each round because it is always solved on a well-defined categorical partition; the partition itself is updated by boosting gradients. In case of ties in the $\arg\max$, we adopt the deterministic first-index rule. If provisional labels collapse to fewer than two classes, we reuse the previous valid projection to keep the stagewise update well-defined.  
...
\begin{itemize}
    ...
    For $m>0$, form current probabilities $p^{(m-1)}$, gradients $g^{(m)} = Y - p^{(m-1)}$, and induced labels $z_i^{(m)}=\arg\max_c g^{(m)}_{i,c}$ (deterministic tie-breaking). Refit LDA on $(X,z^{(m)})$ to obtain $G^{(m)}$; if $z^{(m)}$ contains fewer than two classes, reuse $G^{(m-1)}$.
...
Let $n$ be the number of samples, $p$ the number of features, $C$ the number of classes, and $M$ the number of boosting rounds. A one-shot LDA+GBM pipeline fits LDA once and then trains $M$ boosting stages on fixed transformed features. By contrast, LdaBoost recomputes LDA at each round.

Denoting by $\mathcal{C}_{\mathrm{LDA}}(n,p,C)$ the cost of one LDA fit and by $\mathcal{C}_{\mathrm{tree}}(n,C)$ the cost of one class-wise boosting stage, the overall costs are:
\[
\text{LDA+GBM: } \mathcal{C}_{\mathrm{LDA}}(n,p,C) + M\,\mathcal{C}_{\mathrm{tree}}(n,C),
\]
\[
\text{LdaBoost: } M\,\mathcal{C}_{\mathrm{LDA}}(n,p,C) + M\,\mathcal{C}_{\mathrm{tree}}(n,C).
\]
Thus, the additional overhead of LdaBoost is approximately $(M-1)\,\mathcal{C}_{\mathrm{LDA}}(n,p,C)$ relative to one-shot LDA preprocessing. In practice, this trade-off is more visible in high-dimensional settings, where repeated scatter-matrix computations dominate runtime, and is much smaller when $p$ is modest.
...
For the HAR dataset, we also evaluate the macro F1 score as an accuracy measure for a more complete assessment.
...
\section*{Results}
\subsection*{HAR}
In the HAR dataset (Table~\ref{tab:classification_results}), 10-fold cross-validation shows that the baseline GBM reaches an accuracy of 0.995 (SD = 0.002) when using the full feature set. PCA+GBM drops to 0.942 (SD = 0.006), whereas LDA+GBM and LdaBoost---both relying on five discriminant components instead of the original 561 predictors---achieve 0.978 (SD = 0.004) and 0.978 (SD = 0.006), respectively.

On the held-out test set, LDA+GBM attains the highest accuracy (0.9528), closely followed by LdaBoost (0.9511), while GBM and PCA+GBM reach 0.9400 and 0.9053.

From a computational perspective, HAR highlights an enormous acceleration of one-shot LDA preprocessing in front of GBM: LDA+GBM needs 10.25 seconds versus 675.07 seconds for GBM and 652.62 seconds for PCA+GBM, corresponding to about $65.9\times$ and $63.7\times$ speedups, respectively. This gain comes with only a small cross-validated accuracy reduction versus full-feature GBM (0.978 vs 0.995), while preserving better external validation accuracy (0.9528 vs 0.9400).
...
\footnotesize
\caption{Cross-validated accuracy (mean with SD in parentheses), test accuracy, and test validation runtime (seconds).
...
\midrule
\multirow{5}{*}{\textbf{CV}} 
    & IRIS      & 0.960 (0.044) & 0.967 (0.031) & \textbf{0.980 (0.020)} & 0.967 (0.031) \\
    & YEAST     & 0.568 (0.037) & 0.582 (0.023) & \textbf{0.614 (0.030)} & 0.589 (0.032) \\
    & HAR       & \textbf{0.995 (0.002)} & 0.942 (0.006) & 0.978 (0.004) & 0.978 (0.006) \\
    & RAINFALL  & 0.861 (0.025) & 0.863 (0.015) & 0.860 (0.021) & \textbf{0.864 (0.025)} \\
    & SONAR     & 0.694 (0.173) & \textbf{0.764 (0.016)} & 0.688 (0.052) & 0.721 (0.075) \\
\midrule
\multirow{2}{*}{\textbf{Test}} 
    & HAR       & 0.9400 & 0.9053 & \textbf{0.9528} & 0.9511 \\
    & RAINFALL  & \textbf{0.8790} & 0.8721 & 0.8607 & 0.8584 \\
\midrule
\multirow{2}{*}{\textbf{Test time (s)}} 
    & HAR       & 675.07 & 652.62 & 10.25 & 286.22 \\
    & RAINFALL  & 0.53 & 0.70 & 0.27 & 2.33 \\
\bottomrule
\end{tabular}
\endgroup
...
\textit{Note:} HAR test values use the official holdout split. RAINFALL test values use train-split fallback validation due to missing external test labels.
\end{table}

\begin{figure}[h!]
        \centering
        \includegraphics[width=0.75\linewidth]{figures/accuracy_comparison.pdf}
        \caption{Accuracy comparison across real datasets: LDA-based approaches (LDA+GBM and \textsc{LdaBoost}) are most frequently among the best-performing models.}
...
\bigskip
\noindent
The Yeast dataset (8 features) showed substantial improvements, with cross-validated accuracy rising from 0.568 (SD = 0.037) for GBM to 0.614 (SD = 0.030) with LDA+GBM, while LdaBoost reached 0.589 (SD = 0.032) and PCA+GBM 0.582 (SD = 0.023). This confirms the benefit of incorporating discriminant components in multiclass problems even when the feature space is relatively low dimensional.

With a small number of features such as in the IRIS dataset, LDA+GBM delivered the highest accuracy (0.980, SD = 0.020). LdaBoost and PCA+GBM (both 0.967, SD = 0.031) and GBM (0.960, SD = 0.044) performed similarly.

For the Sonar dataset (binary target, 60 features), the baseline GBM obtained 0.694 (SD = 0.173), LDA+GBM 0.688 (SD = 0.052), and LdaBoost 0.721 (SD = 0.075), while PCA+GBM achieved the best performance at 0.764 (SD = 0.016). In the Rainfall dataset (binary target, 11 features), performances were comparable, with GBM at 0.861 (SD = 0.025), PCA+GBM at 0.863 (SD = 0.015), LDA+GBM at 0.860 (SD = 0.021), and LdaBoost slightly ahead at 0.864 (SD = 0.025).

Regarding test accuracy, for HAR the LDA+GBM model reached 0.9528, LdaBoost 0.9511, GBM 0.9400, and PCA+GBM 0.9053. In the Rainfall dataset, GBM achieved 0.8790, PCA+GBM 0.8721, LDA+GBM 0.8607, and LdaBoost 0.8584.
...
We first begin our simulations with a moderate sample size ($N=1{,}000$). Figures~\ref{fig:fig1} and~\ref{fig:fig2} illustrate the accuracy trends observed in multiclass and binary classification scenarios, respectively, as the number of features varies at an intermediate mean correlation level ($\rho\approx 0.5$). These figures highlight the comparative performance advantages of LdaBoost over classical GBM and show that LdaBoost can maintain robust predictive accuracy as feature dimensionality increases.
...
\renewcommand{\arraystretch}{1.0}
\footnotesize
\caption{Runtime ablation on synthetic scenarios (5 seeds): fit time in seconds and test accuracy, reported as mean (SD).}
\label{tab:runtime_ablation}
\begin{tabular}{@{}llcc@{}}
\toprule
\textbf{Scenario} & \textbf{Model} & \textbf{Fit Time (s)} & \textbf{Test Accuracy} \\
\midrule
\multirow{4}{*}{\textbf{Binary, $p=300$}}
    & GBM      & 2.702 (0.025) & 0.770 (0.032) \\
    & PCA+GBM  & 0.506 (0.012) & 0.761 (0.035) \\
    & LDA+GBM  & 0.077 (0.014) & 0.685 (0.018) \\
    & LdaBoost & 2.683 (0.105) & 0.681 (0.020) \\
\midrule
\multirow{4}{*}{\textbf{Multiclass, $p=100$}}
    & GBM      & 4.497 (0.054) & 0.591 (0.026) \\
    & PCA+GBM  & 1.312 (0.012) & 0.586 (0.018) \\
    & LDA+GBM  & 0.365 (0.005) & 0.536 (0.028) \\
    & LdaBoost & 1.153 (0.010) & 0.541 (0.033) \\
\bottomrule
\end{tabular}
\endgroup
\end{table}
...
Overall, the results show that LdaBoost often yields increased performance gains as classification tasks become more complex in terms of the number of target classes. However, this pattern is less pronounced when the number of features is increased in multiclass settings. In contrast, in binary classification tasks, LdaBoost exhibits a clear advantage as the dimensionality of the features increases in the main tuned analyses. These findings highlight the adaptability and robustness of the LDA-boosting framework across a variety of data structures.
